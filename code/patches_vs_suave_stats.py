...
def histogram_patches_vs_suave(grads_exp, grads_rec_patches, grads_rec_suave, grad_type=grad_type, path_to_data_dir=path_to_data_dir,
    ...
    for i in dim:
        ...
        n_s, _, _ = plt.hist(suave_vals, bins=bins, color="indigo", alpha=0.5, label="CFE")
        n_p, _, _ = plt.hist(patches_vals, bins=bins, color="dimgray", alpha=0.5, label="Standard", zorder=100)
