...
def load_suave_amps(cat_tag, m, b=0.5, rlzs=401, basis='bao_fixed', grad_dir=globals.grad_dir):
    """Return a (n,4) array of the gradient amplitudes recovered with Suave."""
    amps = np.empty((rlzs,4))
    for rlz in range(rlzs):
        suave_dict = np.load(os.path.join(grad_dir, f'suave_data/{cat_tag}/{basis}/{cat_tag}_rlz{rlz}_m-{m:.3f}-L_b-{b:.3f}.npy'), allow_pickle=True).item()
