...
from funcs.suave import cf_model
...
def main(mock_type=globals.mock_type,
            ...
            nbins=globals.nbins, randmult=globals.randmult, data_dir=globals.data_dir, overwrite=False):
    ...
    for i, mock_fn in enumerate(mock_set.mock_fn_list):
        ...
        # check if the 4-param. fit has already been computed for this mock
        if not overwrite:
            if os.path.exists(save_fn):
                print(f'4-parameter fit already computed for mock {mock_fn}! moving to the next.')
                continue
