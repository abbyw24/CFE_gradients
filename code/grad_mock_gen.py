...
for m in m_arr_perL:
    for b in b_arr:
        ...
        a = f"m-{m}-L_b-{b}"
        # save xs
        np.save(f"gradient_mocks/{grad_dim}D/mocks/grad_mock_"+a,xs)
        ...
        plt.legend()
        fig1.savefig(f"gradient_mocks/{grad_dim}D/mocks/grad_mock_{a}.png")
        ...
        np.save(f"gradient_mocks/{grad_dim}D/mocks_colored/clust_"+a, xs_clust_grad)
        np.save(f"gradient_mocks/{grad_dim}D/mocks_colored/unclust_"+a, xs_uncl_grad)
